...
def getPctForestArea():
    ...
    df_income['Pct_Lost'] = df_income['YR2004']-df_income['YR2021']
    df_income.loc[60] = df_income.mean(numeric_only=True)
    df_income['Country'] = df_income['Country'].fillna('Africa')
    df_income.sort_values(by='YR2021', inplace=True,ascending=False)
...
def fig_forest_bar():
    return px.bar(df_percentlost.iloc[0:10, [0,1,18]], x='Country', template=Rabet_bg_color,
                  labels={'Country': 'Country','variable':'Year','value':'Pct Cover'}, y=['YR2004','YR2021'],
